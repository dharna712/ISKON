...
class _Question {
  ...
  final List<VerseWord> options; // for meaning: text = meaning(lang); for blank: text = sa

  _Question({required this.type, required this.word, required this.options});

  /// The label shown for one option, given the current language.
  String label(VerseWord option, AppLang lang) =>
      type == _QType.meaning ? option.meaning(lang) : option.sa;

  bool isCorrect(VerseWord option) => option.sa == word.sa;
}
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  List<_Question> _buildQuiz() {
    ...
    for (final w in meaningWords) {
      ...
      final opts = [w, ...distractors.take(2)]..shuffle(rand);
      questions.add(_Question(type: _QType.meaning, word: w, options: opts));
    }
    for (final w in blankWords) {
      final distractors = words.where((x) => x.sa != w.sa).toList()..shuffle(rand);
      final opts = [w, ...distractors.take(2)]..shuffle(rand);
      questions.add(_Question(type: _QType.blank, word: w, options: opts));
    }
    questions.shuffle(rand);
    return questions;
  }

  void _select(_Question q, VerseWord option) {
    if (_answered) return;
    setState(() {
      _selected = option;
      _answered = true;
      if (q.isCorrect(option)) _score++;
    });
